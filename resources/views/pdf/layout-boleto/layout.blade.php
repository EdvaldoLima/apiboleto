...
#instr_header {
	/* background: url('imagens/codern.png') no-repeat top left; */
	padding-left: 50px;
	height: 80px;
}
...
<div id="container">
	...
	<div id="instr_header">
		...
		<tr>
			<td>
				<img width="100" src="{{asset('imagens/codern.png')}}" alt="">
			</td>
			<td>
				<h1>{{$dadosboleto["identificacao"] }} {{isset($dadosboleto["cpf_cnpj"]) ? $dadosboleto["cpf_cnpj"] : '' }}</h1>
				<address>{{$dadosboleto["endereco_cedente"] }}<br></address>
...
do Sacado</b></div></TD></tr></tbody></table>
		...
		<tbody>
		<tr>
			<td width=150><IMG SRC="{{asset('imagens/logobb.jpg')}}"></td>
			<td width=50>
		...
		<tr><td class="last_line" rowspan="6">
			...
			<tr class="campos">
				<td class="instrucoes" rowspan="5">
					...
					@if(isset($dadosboleto["demonstrativo"]))
						@foreach($dadosboleto["demonstrativo"] as $dem)
							<div>{{$dem}}</div>
						@endforeach
					@endif
					@if(isset($dadosboleto["instrucoes"]))
						@foreach($dadosboleto["instrucoes"] as $ins)
						<div>{{$ins}}</div>
						@endforeach
					@endif
				</td>
		...
		<tr class="campos">
			<td class="sacado2">
				<div>{{$dadosboleto["sacado"]}}</div>
				@if(isset($dadosboleto["endereco"]))
					@foreach($dadosboleto["endereco"] as $ende)
						<div>{{$ende}}</div>
					@endforeach
				@endif
			</td>
